...
def load_jpx_stocks():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            JPX_STOCKS = stocks
    except Exception as e: print("JPX Error:", e)
...
def get_ai_summary(title: str) -> str:
    ...
    payload = {"contents": [{"parts": [{"text": f"金融初心者向けに2行で要約し影響を判定: {title}"}]}]}
    try:
    ...
    if rate == 0.0:
        ...
        try:
            if conn and cursor:
                ...
                old = cursor.fetchone()
                if old: rate = old["price"]; fetch_time = "前回取得値"
        except: pass
...
def handle_message(event):
    ...
    if "会員連携" in text: line_bot_api.reply_message(event.reply_token, TextSendMessage(text="会員番号を送信してください。\n例: AB1234"))
        ...
        try:
            ...
            cursor.close(); conn.close()
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text="連携完了！"))
        except: pass
...
def search_stock(q: str, asset_type: str = "ALL"):
            ...
            try:
                ...
                if res.status_code == 200:
                    for item in res.json().get("results", []):
                        ...
                        if code and (code.endswith(".T") or len(code)==4):
    ...
    if asset_type in ["FUND", "ALL"] and len(results) < 8:
        ...
        if len(results) < 8:
            try:
                res = requests.get(f"https://finance.yahoo.co.jp/api/v1/finance/suggest/realtime?query={urllib.parse.quote(q.replace(' ',''))}", headers=headers, timeout=3)
                if res.status_code == 200:
                    for item in res.json().get("results", []):
                        code = item.get("code", ""); name = item.get("name", "")
                        if len(code) == 8 and code.isalnum() and not code.endswith(".T"):
                            if not any(r["ticker"] == code for r in results): results.append({"ticker": code, "name": unicodedata.normalize('NFKC', name)})
                        if len(results) >= 8: break
            except: pass
...
def get_portfolio(user_id: str):
    ...
    for row in rows:
        ...
        current_price = fetched_price if fetched_price > 0 else (item.get("manual_price") or average_price)
...
def get_jp_news(user_id: str):
    try:
        conn = get_db_connection(); cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute("SELECT name FROM portfolio WHERE user_id = %s", (user_id,))
        p_names = [r["name"] for r in cursor.fetchall()]
        cursor.execute("SELECT name FROM watchlist WHERE user_id = %s", (user_id,))
        w_names = [r["name"] for r in cursor.fetchall()]
        cursor.close(); conn.close()
    except: return []

    target_names = list(set(p_names + w_names)) + ["日経平均", "S&P500"]
